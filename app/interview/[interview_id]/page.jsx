...
const page = () => {
  ...
  const takeinterview = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("Interviews")
        .select("jobPosition, questionlist") // Fetch required data
        .eq("id", Number(interview_id))
        .single();
      if (error) throw error;
      setinterviewdata({
    ...
    <Card className="w-fit h-[calc(75% - 8px)] shadow-md">
      <CardContent className="flex items-center justify-center gap-4 flex-col">
        ...
        <div className="w-full flex items-center justify-center flex-col gap-3">
          {/* MODIFICATION: Use dynamic data */}
          <h1 className="text-xl font-bold">
            {interviewdetails?.jobPosition || "Loading Job Position..."}
          </h1>
          <span className="text-xs font-medium flex gap-2 items-center justify-center">
            <Clock size={15} />
            {interviewdetails?.interviewDuration || "Loading Duration..."}
          </span>
        </div>
        ...
        <div>
          <Button onClick={takeinterview} disabled={loading || !username.trim()}>
            {loading ? <LoaderIcon className="animate-spin" /> : <Play />}
            Start Interview
          </Button>
        </div>
      </CardContent>
